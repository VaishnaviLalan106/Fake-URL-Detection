...
const featureDefinitions = {
  url_length: { weight: 0.1, threshold: 75, description: "URL length > 75 characters is suspicious." },
  ...
  count_dots: { weight: 0.1, threshold: 4, description: "More than 4 dots in the hostname can be used for obfuscation." },
  count_hyphens_domain: { weight: 0.1, threshold: 1, description: "Hyphens in a domain name are sometimes used in phishing." },
  count_at: { weight: 0.5, threshold: 0, description: "The '@' symbol in a URL is highly suspicious and often used to trick users." },
  ...
  has_https: { weight: -0.2, threshold: 1, description: "Lack of HTTPS is a significant security risk." },
  has_ip: { weight: 0.8, threshold: 0, description: "Using an IP address instead of a domain name is a classic phishing technique." },
  count_digits_domain: { weight: 0.1, threshold: 2, description: "More than 2 digits in the domain name can be a sign of a malicious site." },
  suspicious_tld: { weight: 0.4, threshold: 0, description: "The URL uses a Top-Level Domain (TLD) commonly associated with phishing (e.g., .xyz, .top, .info)." },
  suspicious_tokens: { weight: 0.3, threshold: 0, description: "Contains suspicious keywords like 'login', 'secure', 'bank', etc." },
  domain_entropy: { weight: 0.2, threshold: 3.5, description: "High domain entropy (randomness) can indicate a machine-generated, illegitimate domain." },
};

// List of suspicious TLDs
const SUSPICIOUS_TLDS = ['.xyz', '.top', '.info', '.site', '.club', '.online', '.buzz', '.biz', '.link'];

// List of suspicious tokens
const SUSPICIOUS_TOKENS = ['login', 'secure', 'account', 'verify', 'update', 'bank', 'paypal', 'confirm', 'signin', 'webscr', 'auth', 'support', 'service'];
  ...
  for (const key in featureDefinitions) {
    ...
    if (key === 'has_https') {
      ...
      if (value > def.threshold) {
        ...
        featureScore = Math.min(1, value / (def.threshold * 2 + 1));
      }
    }
    ...
    } else if (key === 'has_https') { // special case for has_https to show it when it's good
      const weightedScore = featureScore * def.weight;
      totalScore += weightedScore;
    }
  }
...
export function analyzeUrl(url: string) {
  ...
  const { score: confidence, reasons } = score(features);
